...
export const userQuotaModel = pgTable(
  "UserQuota",
  {
    ...
    // --- Custom (fork-only) quota columns ---
    flowsLimit: integer(),
    flowsUsed: integer().notNull().default(0),
    broadcastsLimit: integer(),
    broadcastsUsed: integer().notNull().default(0),
    aiAgentsEnabled: boolean().notNull().default(true),
    // --- Upstream bot-message quota columns ---
